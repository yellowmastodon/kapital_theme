	...
	if ($featured_post_query->have_posts()):
		while ($featured_post_query->have_posts()):
			...
			<article class="featured-post archive-item alignwider ff-grotesk">
					<div role="link" tabindex="0" onclick="window.location='<?php echo get_post_permalink($post->ID) ?>'" class="row gx-4 gy-3 archive-item-link text-decoration-none">
						...
						<div class="col-12 col-md-6 position-relative">
							<?php render_top_row($post_type, $render_settings["show_date"], $render_settings["show_views"], $post_date, $post->ID, "d-none d-lg-flex") ?>
							...
							<?php render_bottom_row($post_type, $custom_taxonomies, $render_settings["show_categories"], $render_settings["show_author"], $filtered_terms); ?>
						</div>
					</div>
			</article>
		<?php endwhile;
